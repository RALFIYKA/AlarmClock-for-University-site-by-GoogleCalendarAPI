...
def main():
    days, corr, corrtime, ans, creds = ('Понедельник', 'Вторник', 'Среда', 'Четверг', 'Пятница',
                                        'Суббота'), [], [], [], None
    thissite = (requests.get(url='https://eners.kgeu.ru/apish2.php?group=%D0%90%D0%A2-1-21&week=17&type=one').text)
    ...
    for i in range(len(corr)):
        for el in days:
            if len(corr[i]) == 4 and corr[i - 6] == el:
                if len(corr[i - 5]) != 8:
                    ans.append('0' + corr[i - 5][0] + '-' + corr[i - 5][1:3] + '-' + corr[i - 5][3:7] + ' ' + corr[i][:2] + ':' + corr[i][2:])
                else:
                    ans.append(corr[i - 5][:2] + '-' + corr[i - 5][2:4] + '-' + corr[i - 5][4:8] + ' ' + corr[i][:2] + ':' + corr[i][2:])
